...
{
    /**
     * List of blocked/private IP ranges for SSRF protection
     */
    private function getBlockedHosts()
    {
        return [
            // Private IPv4 ranges
            '127.0.0.1', 'localhost',
    ...
    private function validateWebhookUrl($url)
    {
        ...
        if (in_array($host, ['localhost', '127.0.0.1', '::1', '[::1]'])) {
            return false;
        }
        ...
        if ($ip === $host) {
            // Resolution failed - might still be valid, but block if it looks like an IP
