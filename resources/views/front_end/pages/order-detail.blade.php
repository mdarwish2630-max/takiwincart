      ...
      <div class="md:container w-full mx-auto px-4">
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-6">
                ...
                <table class="min-w-full text-sm text-gray-700">
                  <thead class="bg-gray-50 border-b text-gray-800 font-medium">
                    ...
                    @if ($order['order_status'] == 1)
                        <th class="text-right px-4 py-3">{{ __('Download') }}</th>
                    @endif
                    </tr>
                  </thead>
                  <tbody>
                            ...
                            <td class="px-4 py-3 justify-center text-gray-600">
                              <div class="flex gap-2 justify-center">
                                @if (!empty($download_product->downloadable_product))
                                  <a href="{{ get_file($download_product->downloadable_product) }}" type="button" class="btn-primary p-2 h-10 w-10 flex items-center justify-center download_prod_{{ $item['product_id'] }}" title="{{ __('Download') }}">
                                    <i class="fas fa-download"></i>
                                  </a>
                                @endif
                                  <a href="{{ get_file($download_product->downloadable_product) }}" data-product-id="{{ $item['product_id'] }}" class="btn-primary p-2 h-10 w-10 flex items-center justify-center downloadable_product_variant" download title="{{ __('Download') }}">
            ...
            <!-- Billing Information Only (no shipping for digital) -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Customer Information -->
              <div class="card p-4">
                <h3 class="font-semibold text-gray-800 mb-4">{{ $themeSettings['order_info_address_bill'] ?? __('Customer Information') }}</h3>
              ...
              <!-- Payment Information -->
              <div class="card p-4">
                <h3 class="font-semibold text-gray-800 mb-4">{{ __('Payment Information') }}</h3>
                <ul class="space-y-2 text-sm">
                  <li class="flex gap-3">
                    <span class="font-semibold md:w-[150px] w-[100px] text-gray-900">{{ __('Payment Type') }}</span>
                    <span class="text-gray-800">{{ $order['paymnet_type'] }}</span>
                  </li>
                  <li class="flex gap-3">
                    <span class="font-semibold md:w-[150px] w-[100px] text-gray-900">{{ __('Order Status') }}</span>
          ...
          <div class="space-y-6">
            ...
            <div class="card p-4">
              ...
              <ul class="space-y-3 text-sm">
                ...
                {{-- Delivery charges removed - not applicable for digital products --}}
                <li class="flex justify-between">
            ...
            <div class="card p-4 bg-blue-50 border-blue-200">
              <h3 class="font-semibold text-blue-800 mb-2 flex items-center gap-2">
                <i class="fas fa-cloud-download-alt"></i> {{ __('Digital Product') }}
              </h3>
              <p class="text-sm text-blue-700">{{ __('Your digital files will be available for download once the order status is "Delivered". Check the download button next to each item above.') }}</p>
            </div>
