            ...
            <div class="col-lg-4 col-md-5 col-12">
                <div class="cart-variant-img">
                    <div class="variant-main-media">
                        <img src="{{ asset((isset($product->cover_image_path) && !empty($product->cover_image_path)) ? $product->cover_image_path : 'default_img.png') }}" class="default-img"  width="100%" target="_blank" alt="logitech Keys">
                    </div>
                </div>
            </div>
            <div class="col-lg-8 col-md-7 col-12">
                <div class="cart-variant-detail">
                    <span class="ctg-badge">{{ isset($product->categories) && !empty($product->categories) ? $product->categories->name : '' }}</span>
                    <h3>{{ $product->name }}</h3>
                    ...
                    @foreach ($variant as $key => $value)
                        ...
                        @if ($variation_option == 1)
                            <label for="pro_variants_name" class="pt-2">{{ ucfirst($attribute->name) }}</label>
                            <select name="product[{{ $key }}]" id="pro_variants_name_{{$key}}" class="form-control custom-select variant-selection pro_variants_name{{ $key }}">
                    ...
                    <div class="cart-variable row pt-3">
                        ...
                        <div class="col-md-6">
                            <div class="variation_price1 text-end" style=" font-size: large; ">
                                ...
                                @else
                                    <p>{{ currency_format_with_sym($product->price, getCurrentStore()) }}</p>
...
<div class="modal-footer pb-0">
    ...
    <a href="#!" type="submit" class="btn btn-primary add_to_cart_variant toacartvariant btn-badge mx-1" data-toggle="tooltip" data-id="{{ $product->id }}" >
        {{ __('Add To Cart') }}
