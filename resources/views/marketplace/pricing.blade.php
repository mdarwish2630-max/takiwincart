...
<script src="{{ asset('js/jquery.min.js') }}"></script>
    ...
    @if(module_is_active('GoogleCaptcha') && (isset($admin_settings['RECAPTCHA_MODULE']) ? $admin_settings['RECAPTCHA_MODULE'] : 'no') == 'yes' )
        @if (isset($admin_settings['NOCAPTCHA_VERSON']) && $admin_settings['NOCAPTCHA_VERSON'] == 'v2')
                {!! NoCaptcha::renderJs() !!}
        @else
            <script src="https://www.google.com/recaptcha/api.js?render={{ $admin_settings['NOCAPTCHA_SITEKEY'] }}"></script>
            <script>
                $(document).ready(function() {
                    grecaptcha.ready(function() {
                        grecaptcha.execute('{{ $admin_settings['NOCAPTCHA_SITEKEY'] }}', {
                            action: 'submit'
                        }).then(function(token) {
