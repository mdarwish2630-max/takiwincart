...
$(document).on('click', '.show_confirm', function (e) {
    ...
    var yes = $(this).attr("data-text-yes");
    var no = $(this).attr("data-text-no");
        ...
        $('.summernote-simple').summernote({
            ...
            callbacks: {
                onChange: function(contents, $editable) {
                    // Optional: Strip <p> if there's only plain text inside
                    const isWrappedInP = contents.trim().match(/^<p>(.*?)<\/p>$/);
                    if (isWrappedInP && !isWrappedInP[1].includes('<')) {
                        $(this).summernote('code', isWrappedInP[1]);
                    }
                }
            }
        });
        ...
        $('.summernote-simple').summernote({
            ...
            callbacks: {
                onChange: function(contents, $editable) {
                    ...
                    const isWrappedInP = contents.trim().match(/^<p>(.*?)<\/p>$/);
                    if (isWrappedInP && !isWrappedInP[1].includes('<')) {
                        $('.summernote-simple').summernote('code', isWrappedInP[1]);
                    }
                }
            }
        });
...
$(".external-left-btn .show-btn").click(function () {
    $(".external-left-btn").toggleClass("active");
});
